...
def _save_risk_metrics(db: Session, fund_code: str, risk_metrics: dict):
    ...
    if risk_record:
        ...
        risk_record = FundRiskMetrics(
            fund_code=fund_code, **{k: v for k, v in risk_metrics.items() if hasattr(FundRiskMetrics, k)}
        )
        db.add(risk_record)
...
def _latest_cached_nav_date(db, fund_code):
    ...
    row = (
        db.query(FundNavHistory.trade_date)
        .filter(FundNavHistory.fund_code == fund_code)
        .order_by(desc(FundNavHistory.trade_date))
        .first()
    )
...
def _load_nav_history_rows(db, fund_code):
    ...
    rows = (
        ...
        .order_by(FundNavHistory.trade_date.asc())
        .all()
    )
...
def _nav_cache_is_fresh(db, fund_code):
    ...
    if not latest_cached:
        return False
    return not (latest_snapshot and latest_cached < str(latest_snapshot))
