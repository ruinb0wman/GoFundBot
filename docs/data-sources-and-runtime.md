...
| **Python 3** | `Scripts/cli/` | 工具脚本，由 `pythonRunner.ts` 通过 `child_process.spawn()` 调用 |
...
| **data_complete** | `Scripts/cli/data_complete.py` | `akshare` Python 库 | A 股列表、行业板块映射 |
...
| 函数 | 文件 | 职责 |
|------|------|------|
| `computeRiskMetrics` | `riskMetricsService.ts` | 最大回撤、夏普比率、波动率、Calmar 比率 |
| `classifyFundIndustry` | `industryService.ts:22` | 正则匹配基金名称→行业标签（19 个类别） |
| `enrichFund` | `screeningEnrichment.ts:14` | 聚合 NAV + 风险指标 + 行业分类→基金富化数据 |

### 3.5 AI 对话服务

| 函数/类 | 文件 | 职责 |
|---------|------|------|
| `SkillRouter` | `chatSkills.ts:256` | 意图路由（关键词匹配 + LLM 路由），支持 7 种技能 |
| `chatTools` 函数集 | `chatTools.ts` | Function Calling 工具集（查基金、行情、筛选等） |
| `searchWeb` | `searchService.ts:137` | 搜索引擎链（Bocha→Tavily→DDG 自动降级） |
| `AI Analyst` | `aiAnalyst.ts` | AI 基金分析 |

### 3.6 研究服务 (`Service/src/services/researchService.ts`)

| 函数 | 职责 |
|------|------|
| `getDashboard` | 研究仪表盘 |
| `getEtfTracking` | ETF 跟踪误差排行 |
| `getIndustryPerformance` | 行业板块表现 |
| `getSectorSummary` | 板块汇总 |
| `rebuildIndustryPerformance` | 重建行业表现缓存 |
...
### 4.1 回测 (`Scripts/cli/backtest.py`)
...
### 4.2 基金爬取 (`Scripts/cli/fetch_fund.py`)
...
### 4.3 数据补全 (`Scripts/cli/data_complete.py`)
