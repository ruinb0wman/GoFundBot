from services.helpers import _json_loads, _to_float
from services.stock_utils import (
    _normalize_stock_code, _is_us_stock_code, _is_a_share_stock_code,
    _is_hk_holding_item, _safe_ratio, _portfolio_holding_items,
    _fund_name_or_type_text, _detect_market_from_code,
)
...
def _fund_text_industry_fallback(text):
    ...
    if fund_type:
        return {'name': fund_type, 'ratio': 0.0, 'count': 0,
                'basis': 'fund_type', 'source': 'fund_name_rule',
                'matched_keyword': type_keyword}

    market_topic, market_keyword = _match_keyword_rule(text, MARKET_TOPIC_RULES)
    if market_topic:
        return {'name': market_topic, 'ratio': 0.0, 'count': 0,
                'basis': 'market_region', 'source': 'fund_name_rule',
                'matched_keyword': market_keyword}

    if _is_broad_index_fund_text(text):
        return {'name': '宽基指数', 'ratio': 0.0, 'count': 0,
                'basis': 'broad_index_name', 'source': 'fund_name_rule'}

    topic, topic_keyword = _match_keyword_rule(text, TOPIC_RULES)
    if topic:
        return {'name': topic, 'ratio': 0.0, 'count': 0,
                'basis': 'index_topic' if ('指数' in text or 'ETF' in text.upper()) else 'fund_name_topic',
                'source': 'fund_name_rule', 'matched_keyword': topic_keyword}

    if _is_etf_feeder_text(text):
        return {'name': '指数联接', 'ratio': 0.0, 'count': 0,
                'basis': 'index_topic', 'source': 'fund_name_rule'}
    return None
...
def _build_portfolio_industry_tag(portfolio: dict):
    ...
    if not holdings:
        return fallback_tag or {'name': '混合型', 'ratio': 0.0, 'count': 0,
                                'basis': 'mixed', 'source': 'top_stock_holdings',
    ...
    for item in holdings:
        industry = (
            item.get('industry') or item.get('industry_name')
            or item.get('industryName') or item.get('sector')
        ...
        bucket = buckets.setdefault(industry, {
            'name': industry, 'ratio': 0.0, 'ratio_count': 0, 'count': 0, 'stocks': [],
        })
        if ratio_available:
            bucket['ratio'] += ratio
    ...
    if fallback_tag:
        ...
        fallback.update({
            'ratio': round(top_by_ratio['ratio'], 2), 'count': top_by_ratio['count'],
            'top_share': round(top_share, 2),
            'count_share': round(top_by_ratio['count'] / valid_count * 100, 2) if valid_count else 0.0,
            'valid_count': valid_count, 'has_weight': total_ratio_count > 0,
            'holding_top_industry': top_by_ratio['name'],
        })
        return fallback

    return {
        'name': '混合型', 'ratio': round(top_by_ratio['ratio'], 2),
        'count': top_by_ratio['count'],
        'basis': 'mixed', 'source': 'top_stock_holdings',
        'top_share': round(top_share, 2),
        'count_share': round(top_by_ratio['count'] / valid_count * 100, 2) if valid_count else 0.0,
