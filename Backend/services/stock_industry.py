...
from services.stock_utils import (
    _is_a_share_stock_code,
    _is_us_stock_code,
    _market_hint_from_holdings,
...
def warm_stock_industry_dictionary(db, fund_codes=None, force=False, limit=None):
    ...
    fetched_map, failed_codes = _fetch_stock_industry_batch(
        db, missing_codes, force_refresh=force, timeout=8.0 if force else 5.0
    )
    db.commit()
    return {
...
def build_stock_industry_dictionary_from_akshare(db, force=False, board_limit=None, stock_limit=None):
    ...
    try:
        result = _build_stock_industry_dictionary_from_sina(
            db, ak, force=force, board_limit=board_limit, stock_limit=stock_limit
        )
        ...
        result = _build_stock_industry_dictionary_from_em_boards(
            db, ak, board_df, force=force, board_limit=board_limit, stock_limit=stock_limit
        )
    ...
    for index, board in enumerate(boards, 1):
        ...
        for _, stock_row in cons_df.iterrows():
            ...
            record.stock_name = str(
                _akshare_row_value(stock_row, 2, "名称", "股票名称", "name") or record.stock_name or ""
            )
    ...
    for index, board in enumerate(boards, 1):
        ...
        for _, stock_row in cons_df.iterrows():
            ...
            if not record:
                ...
                continue
            record.stock_name = str(
                stock_row.get("name") if hasattr(stock_row, "get") else None or record.stock_name or ""
            )
            record.industry = board["name"]
            record.region = record.region or ""
            concepts = _json_loads(record.concepts_json, [])
