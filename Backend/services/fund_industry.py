...
from models import (
    FundIndustryTag, FundPortfolio, FundBasicInfo,
    FundIndustryPerformance, DataFetchTask, StockIndustry,
)
from services.helpers import _json_dumps, _json_loads, _to_float, _normalize_fund_code, _stats_numbers
from services.stock_utils import (
    _normalize_stock_code, _portfolio_holding_items, _safe_ratio,
    _is_us_stock_code, _is_hk_holding_item, _detect_market_from_code,
)
from services.stock_industry import _resolve_stock_industries, _fetch_stock_industry_batch, _collect_stock_codes_from_portfolios
from services.industry_classification import _build_portfolio_industry_tag, _fund_text_industry_fallback, _fund_name_or_type_text
...
def _enhance_portfolio_industries(db, portfolio, force_refresh=False):
    ...
    industry_map, unresolved = _resolve_stock_industries(
        db, all_holdings, force_refresh=force_refresh, allow_network=force_refresh,
    )
...
def _refresh_fund_industry_tag(db, fund_code, fetch_holdings_if_missing=False, force_stock_refresh=False, allow_stock_network=False):
    fund_code = _normalize_fund_code(fund_code)
    if not fund_code:
        return None
...
def batch_refresh_fund_industry_tags(
    db, fund_codes=None, task_id=None, force=False,
    limit=None, build_full_dictionary=False, allow_missing_stock_network=False,
):
    ...
    if build_full_dictionary:
        _set_screening_progress(db, task_id, message='构建全市场股票行业字典...', target_count=total,
    ...
    _set_screening_progress(db, task_id, message='读取本地股票行业字典...', target_count=total,
                            current_count=0, success_count=0, fail_count=0, current_item='')
    dictionary_result = {
        'portfolio_stock_total': len(_collect_stock_codes(db, codes)),
        'mapped_stock_total': db.query(StockIndustry).filter(
            StockIndustry.industry.isnot(None), StockIndustry.industry != '',
        ).count(),
    }
    ...
    if allow_missing_stock_network and (not build_full_dictionary or full_dictionary_ok):
        ...
        if missing_a_share_codes:
            ...
            fetched_map, failed_codes = _fetch_stock_industry_batch(db, missing_a_share_codes, force_refresh=False, timeout=5.0)
            db.commit()
    ...
    success = 0
    fail = 0
    _set_screening_progress(db, task_id, message='刷新基金行业标签...', target_count=total,
                            current_count=0, success_count=0, fail_count=0, current_item='')
    ...
    for index, code in enumerate(codes, 1):
        ...
        try:
            tag = _refresh_fund_industry_tag(db, code, fetch_holdings_if_missing=True,
                                             force_stock_refresh=False, allow_stock_network=False)
            if tag:
                success += 1
            else:
                fail += 1
...
def _build_fund_industry_exposure(db, fund_code, force_refresh=False):
    ...
    industry_map, unresolved = _resolve_stock_industries(db, holding_items, force_refresh=force_refresh, allow_network=force_refresh)
    ...
    for item in holding_items:
        ...
        enriched_item = {**item, 'industry': info.get('industry'), 'region': info.get('region'), 'concepts': info.get('concepts') or []}
    ...
    _upsert_fund_industry_tag(db, fund_code, result['industry_tag'],
...
def _screening_industry_context(db, fund_codes):
    ...
    tag_map = {
        item.fund_code: {
            'name': item.industry_tag, 'ratio': item.industry_ratio,
            'count': item.industry_count, 'basis': item.basis, 'source': item.source,
        }
    ...
    if stock_codes:
        ...
        industry_lookup = {record.stock_code: {'industry': record.industry, 'stock_name': record.stock_name} for record in records}
...
def rebuild_industry_performance_stats(db):
    ...
    for tag, basic in rows:
        ...
        bucket = grouped.setdefault(industry, {
            'funds': [], 'return_3m': [], 'return_6m': [], 'return_1y': [], 'return_3y': [],
        })
        bucket['funds'].append({'fund_code': basic.fund_code, 'fund_name': basic.fund_name, 'fund_type': basic.fund_type})
    ...
    for industry, bucket in grouped.items():
        ...
        record.detail_json = _json_dumps({
            'period_counts': {
                '3m': stat_3m['count'], '6m': stat_6m['count'],
                '1y': stat_1y['count'], '3y': stat_3y['count'],
            },
...
def _industry_performance_payload(db):
    ...
    return {
        ...
        'top_3m': sorted(items, key=lambda x: x.get('return_3m_median') if x.get('return_3m_median') is not None else -9999, reverse=True)[:8],
        'top_1y': sorted(items, key=lambda x: x.get('return_1y_median') if x.get('return_1y_median') is not None else -9999, reverse=True)[:8],
        'weak_3m': sorted(items, key=lambda x: x.get('return_3m_median') if x.get('return_3m_median') is not None else 9999)[:8],
    }
