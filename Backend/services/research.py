...
from models import (
    FundBasicInfo, FundRiskMetrics, FundScreeningRank, FundEstimate,
    FundEtfTracking, DataFetchTask,
)
from services.helpers import (
    _json_loads, _json_dumps, _round_or_none, _median, _avg, _to_float,
    _positive_rate, _normalize_fund_code,
)
from services.data_service_client import get_data_service_client, DataServiceError
from services.fund_industry import _industry_performance_payload, rebuild_industry_performance_stats
from models import FundIndustryTag
...
def _build_research_market_stats(db):
    ...
    for item in items:
        ...
        type_stat = type_map.setdefault(fund_type, {
            "fund_type": fund_type, "count": 0, "pass_4433": 0,
            "return_1y_values": [], "return_3m_values": [],
        })
        type_stat["count"] += 1
    ...
    for stat in type_map.values():
        type_stats.append({
            "fund_type": stat["fund_type"], "count": stat["count"],
    ...
    return {
        "summary": {
            "total_funds": total, "risk_ready": risk_ready,
...
def _refresh_etf_tracking_from_akshare(db, limit=500):
    ...
    params = {
        "pn": "1", "pz": str(max(limit, 100)), "po": "1", "np": "1",
...
def _build_etf_tracking_from_cache(limit=80):
    ...
    for fund in getattr(cache, 'fund_list', []) or []:
        ...
        if code and 'ETF' in name.upper():
            funds.append({
                'fund_code': code, 'fund_name': name, 'fund_type': 'ETF',
                'estimate_change': None, 'return_1y': None, 'nav_date': None,
    ...
    return {
        ...
        'categories': [],
        'summary': {
            'total': len(funds), 'with_estimate': 0,
            'avg_estimate_change': None, 'positive_estimate_rate': None,
            'net_flow_available': False, 'source': 'fund_list_cache',
            'stale': True,
...
def _build_etf_tracking_snapshot(db, limit=80, refresh=False):
    ...
    for row in rows:
        items.append({
            ...
            'fund_type': 'ETF', 'latest_price': row.latest_price,
            'iopv': row.iopv, 'discount_rate': row.discount_rate,
            'estimate_change': row.change_percent,
            'change_percent': row.change_percent,
            'change_amount': row.change_amount, 'volume': row.volume,
            'amount': row.amount, 'turnover_rate': row.turnover_rate,
            'fund_share': row.fund_share, 'market_value': row.market_value,
            'return_1y': None,
    ...
    category_items = [
        {
            ...
            'return_1y_median': None, 'net_flow': None,
        }
...
def _build_research_etf_tracking(db, limit=80):
    ...
    for item in etf_items:
        ...
        stat = category_stats.setdefault(category, {
            ...
            "estimate_values": [], "return_1y_values": [],
        })
        stat["count"] += 1
        stat["estimate_values"].append(item.get("estimate_change"))
...
def _build_research_sector_summary(limit=50):
    ...
    return {
        ...
        'summary': {
            ...
            'positive_count': sum(1 for it in items if it.get('change_percent') is not None and it['change_percent'] >= 0),
            'negative_count': sum(1 for it in items if it.get('change_percent') is not None and it['change_percent'] < 0),
        },
    }
