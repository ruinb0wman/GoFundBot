...
from services.helpers import (
    _value_to_string, _normalize_date, _json_dumps, _json_loads,
    _trend_daily_return, _first_present, _estimate_is_after_nav,
)
...
def _upsert_fund_estimate(
    db, fund_code,
    name=None, net_worth=None, net_worth_date=None,
    estimate_value=None, estimate_change=None, estimate_time=None,
):
    ...
    if estimate_record:
        ...
        estimate_record = FundEstimate(
            fund_code=fund_code, name=name,
            net_worth=net_worth, net_worth_date=net_worth_date,
            estimate_value=estimate_value, estimate_change=estimate_change,
...
def _refresh_fund_detail_estimate_snapshot(db, fund_code, result=None):
    ...
    trend_rows = _fund_api.cleaner.clean_array_data(
