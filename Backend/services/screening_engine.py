...
from models import (
    FundBasicInfo, FundScreeningRank, FundRiskMetrics,
    FundPortfolio, FundIndustryTag, DataFetchTask,
)
from services.helpers import (
    _normalize_fund_code, _json_dumps, _json_loads, _to_float,
)
from services.data_service_client import DataServiceError, get_data_service_client
from services.risk_metrics import calculate_risk_metrics, _save_risk_metrics
from services.fund_industry import _refresh_fund_industry_tag, _fund_codes_needing_industry_refresh
...
screening_update_status = {
    ...
    'current_fund': '', 'success_count': 0, 'fail_count': 0,
    'start_time': None, 'message': '',
}
...
def _save_screening_snapshot_item(db, item, type_lookup):
    ...
    basic_info = {
        ...
        'fund_type': fund_type, 'net_worth': item.get('nav'),
        'net_worth_date': item.get('navDate'),
        'fee': item.get('fee'), 'source': item.get('source'),
    }
...
def _fetch_screening_snapshot_items(limit=None, db=None, task_id=None):
    ...
    for idx, type_code in enumerate(SCREENING_SNAPSHOT_TYPES, 1):
        if max_count and len(items) >= max_count:
            break
        t1 = time.time()
        _set_screening_progress(db, task_id, message=f"获取排行 ({idx}/{total_types}): {type_code}", current_count=len(items))
...
def calculate_same_type_rankings(db):
    fund_types = db.query(FundBasicInfo.fund_type).filter(
        FundBasicInfo.fund_type.isnot(None), FundBasicInfo.fund_type != ''
    ).distinct().all()
    fund_types = [ft[0] for ft in fund_types]

    for fund_type in fund_types:
        funds = db.query(FundBasicInfo).filter(
            FundBasicInfo.fund_type == fund_type, FundBasicInfo.performance_json.isnot(None)
        ).all()
        if len(funds) < 2:
            continue
        ...
        periods = [
            ('return_1m', 'rank_pct_1m'), ('return_3m', 'rank_pct_3m'),
            ('return_6m', 'rank_pct_6m'), ('return_1y', 'rank_pct_1y'),
            ('return_2y', 'rank_pct_2y'), ('return_3y', 'rank_pct_3y'),
        ]
        ...
        for fund_code, ranks in fund_ranks.items():
            ...
            rank_record.pass_4433 = 1 if check_4433_rule(
                ranks.get('rank_pct_1y'), ranks.get('rank_pct_2y'), ranks.get('rank_pct_3y'),
                None, ranks.get('rank_pct_6m'), ranks.get('rank_pct_3m'),
            ) else 0
...
def _create_data_fetch_task(db, task_type, options=None, message=''):
    task = DataFetchTask(
        ...
        success_count=0, fail_count=0, current_item='',
        message=message,
        options_json=_json_dumps(options or {}),
        started_time=datetime.now(), updated_time=datetime.now(),
    )
    db.add(task)
    db.commit()
    db.refresh(task)
...
def _task_to_update_status(task):
    ...
    if task.status == 'running':
        for mem_key, result_key in [
            ...
            ('current_fund', 'current_fund'), ('success_count', 'success_count'),
...
def batch_update_fund_data(
    fund_types=None, limit=None, mode='sync_nav', task_id=None,
    industry_limit=None, tasks=None, build_industry_dictionary=True,
):
    ...
    try:
        if not task_id:
            task = _create_data_fetch_task(db, 'screening_update', {
                'fund_types': fund_types or [], 'limit': limit,
                'mode': mode, 'industry_limit': industry_limit,
                'build_industry_dictionary': build_industry_dictionary, 'tasks': tasks,
            }, message='获取批量排行...')
            task_id = task.id
        ...
        screening_update_status.update({
            'running': True, 'progress': 0, 'total': 0, 'current_fund': '',
            'success_count': 0, 'fail_count': 0, 'start_time': datetime.now(),
            'message': '获取批量排行...',
        })
        ...
        if update_basic:
            ...
            if limit:
                try:
                    ...
                    pass
            fund_list = [{'code': item.fund_code, 'name': item.fund_name, 'type': item.fund_type} for item in query.all()]
        ...
        if update_basic:
            _set_screening_progress(db, task_id, message='写入基础数据...', target_count=total_to_save,
                                    current_count=0, success_count=0, fail_count=0, current_item='')
            for index, fund in enumerate(fund_list, 1):
                if screening_stop_flag:
                    _set_screening_progress(db, task_id, status='stopped', message=f"已手动停止。成功: {success_count}, 失败: {fail_count}")
                ...
                screening_update_status.update({
                    'progress': index, 'current_fund': f"{fund_code} - {fund.get('name', '')}",
                    'success_count': success_count, 'fail_count': fail_count,
                })
                db.commit()
                _set_screening_progress(db, task_id, current_count=index,
        ...
        if not screening_stop_flag and calculate_risk_task:
            ...
            nav_success = 0
            nav_fail = 0
            _set_screening_progress(db, task_id, message='补齐候选净值并计算风险指标...',
                                    target_count=len(candidates), current_count=0, current_item='')
        ...
        if not screening_stop_flag and refresh_industry_task:
            ...
            industry_codes = [_normalize_fund_code(item.get('code')) for item in fund_list if _normalize_fund_code(item.get('code'))]
            industry_result = batch_refresh_fund_industry_tags(
                db, fund_codes=industry_codes, task_id=task_id, force=True,
                limit=industry_limit, build_full_dictionary=build_industry_dictionary,
        ...
        _set_screening_progress(db, task_id, status='finished', message=message,
                                success_count=success_count, fail_count=fail_count, current_item='')
        screening_update_status['running'] = False
        return {'success': True, 'total': screening_update_status['total'],
                'success_count': success_count, 'fail_count': fail_count}
...
def batch_fill_risk_metrics(db=None, task_id=None):
    ...
    try:
        ...
        screening_update_status.update({
            'running': True, 'progress': 0, 'total': len(missing_codes),
            'current_fund': '', 'success_count': 0, 'fail_count': 0, 'message': '补充风险指标...',
        })
        _set_screening_progress(db, task_id, status='running', message='补充风险指标...',
                                target_count=len(missing_codes), current_count=0, success_count=0, fail_count=0)
        ...
        for idx, code in enumerate(missing_codes, 1):
            ...
            current_item = f"{code}"
            screening_update_status.update({
                'progress': idx, 'current_fund': current_item,
            ...
            db.commit()
            _set_screening_progress(db, task_id, current_count=idx, current_item=current_item,
                                    success_count=success, fail_count=fail)
        ...
        _set_screening_progress(db, task_id, status='finished', message=f"补充完成。成功{success}, 失败{fail}",
                                success_count=success, fail_count=fail)
        screening_update_status['running'] = False
