...
class TestDataServiceClient(unittest.TestCase):
    def setUp(self):
        self.client = DataServiceClient(base_url="http://test:3100/api", timeout=5)

    @patch.object(DataServiceClient, "_get")
    ...
    def test_request_timeout_raises_data_service_error(self):
        with patch.object(self.client.session, "get") as mock_get:
            ...
            with self.assertRaises(DataServiceError) as ctx:
                self.client.health()
    ...
    def test_drop_none_filters_none_values(self):
        result = DataServiceClient._drop_none({"a": 1, "b": None, "c": "hello"})
