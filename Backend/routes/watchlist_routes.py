...
from models import FundWatchlist, FundWatchlistGroup, FundTrend, FundEstimate
from services.helpers import (
    _normalize_fund_code, _value_to_string, _normalize_date,
    _estimate_is_after_nav, _json_loads, _trend_daily_return,
)
from services.estimate_service import (
    _upsert_fund_estimate, _refresh_single_fund_estimate,
    _latest_nav_from_fund_trend, _sync_latest_official_nav,
)
from services.data_service_client import DataServiceClient
...
def get_watchlist():
    ...
    for item in watchlist:
        ...
        funds_data.append({
            ...
            'fund_type': item.fund_type, 'group_id': item.group_id,
            'sort_order': item.sort_order,
            'created_time': item.created_time.isoformat() if item.created_time else None,
            'net_worth': net_worth, 'net_worth_date': net_worth_date,
    ...
    return jsonify({'groups': groups_data, 'data': funds_data})
...
def add_to_watchlist():
    ...
    new_item = FundWatchlist(fund_code=fund_code, fund_name=fund_name, fund_type=fund_type, group_id=group_id, sort_order=new_order)
    ...
    try:
        ...
        if estimate_payload:
            estimate_result = _upsert_fund_estimate(
                db, fund_code, name=estimate_payload.get('name') or fund_name,
        ...
        return jsonify({'message': 'Fund added to watchlist', 'fund_code': fund_code,
                        'sort_order': new_order, 'estimate': estimate_result}), 201
...
def batch_delete_from_watchlist():
    ...
    db = get_db()
    try:
        deleted_count = db.query(FundWatchlist).filter(FundWatchlist.fund_code.in_(fund_codes)).delete(synchronize_session=False)
        db.commit()
...
def create_group():
    ...
    try:
        db.add(new_group)
        db.commit()
        return jsonify({'message': 'Group created', 'group': {'id': new_group.id, 'name': new_group.name, 'sort_order': new_group.sort_order}}), 201
    except Exception as e:
        db.rollback()
        return jsonify({'error': str(e)}), 500
...
def refresh_watchlist_estimates():
    ...
    return jsonify({'message': f'Updated {updated_count} funds', 'updated': updated_count,
                    'total': len(fund_codes), 'failed': failed_count, 'data': results})
