...
from models import (
    FundBasicInfo, FundTrend, FundEstimate, FundPortfolio, FundExtraData,
    FundRiskMetrics, FundNavHistory, FundIndustryTag, FundScreeningRank,
)
...
from services.helpers import (
    _json_dumps, _json_loads, _normalize_fund_code, _value_to_string,
    _normalize_date, _estimate_is_after_nav, is_data_fresh, _to_float,
    _round_or_none,
)
from services.stock_utils import _normalize_stock_code
from services.fund_industry import (
    _upsert_fund_industry_tag, _enhance_portfolio_industries,
    _build_fund_industry_exposure,
)
from services.estimate_service import (
    _upsert_fund_estimate, _sync_latest_official_nav,
    _refresh_fund_detail_estimate_snapshot, _latest_nav_from_fund_trend,
)
...
def _validate_data_service_fund_quality(mapped_data: dict):
    ...
    has_perf = any(perf.get(k) is not None for k in ('1_year_return', '1_month_return', '3_month_return', '6_month_return'))
    if not has_perf:
...
def _get_fund_detail_from_data_service(fund_code):
    ...
    result['_data_source'] = {
        "mode": "data_service", "mapped": True,
        "completeness_score": 70, "replacement_tier": "tier2_gray_validation",
    }
    return result
...
def _build_cached_response(db, fund_code):
    ...
    if estimate:
        ...
        data['realtime_estimate'] = {
            'name': estimate.name, 'fund_code': fund_code,
            'net_worth': estimate.net_worth, 'net_worth_date': estimate.net_worth_date,
    ...
    if portfolio:
        ...
        _upsert_fund_industry_tag(db, fund_code, data['fund_industry_tag'],
...
def _sync_fund_industry_response(db, fund_code, data, source='fund_detail_response'):
    ...
    if portfolio:
        ...
        if tag:
            _upsert_fund_industry_tag(db, fund_code, tag,
    ...
    if persisted_tag:
        data['fund_industry_tag'] = {
            'name': persisted_tag.industry_tag, 'ratio': persisted_tag.industry_ratio,
            'count': persisted_tag.industry_count, 'basis': persisted_tag.basis,
    ...
    try:
        ...
        if basic_record:
            ...
            basic_record = FundBasicInfo(
                fund_code=fund_code, fund_name=basic_info.get('fund_name', ''),
                fund_type=basic_info.get('fund_type', ''), return_1y=return_1y_val,
                basic_json=_json_dumps(basic_info), performance_json=_json_dumps(performance),
            )
            db.add(basic_record)
...
def proxy_eastmoney(subpath):
    ...
    try:
        ...
        for impersonate_target in ("chrome124", "chrome120", "chrome110", "chrome101", "edge101"):
            try:
                resp = curl_requests.get(target_url, headers=base_headers, timeout=10, verify=False, impersonate=impersonate_target)
                if resp.status_code == 200:
                    return jsonify(resp.json())
            except Exception:
                continue
    ...
    try:
        ...
        if ds_items:
            funds = [{
                ...
                'TYPE': item.get('type', ''), 'PINYIN': item.get('pinyin', ''),
        ...
        if ds_try is not None:
            ...
            if quality_passed:
                ...
                ds_result['_data_source'] = {
                    "mode": "auto", "used": "data_service",
                    "fallback": False, "quality_passed": True, "quality_issues": [],
                }
    ...
    if fund_data:
        ...
        if basic_record:
            ...
            try:
                ...
                pass
            basic_record = FundBasicInfo(
                fund_code=fund_code, fund_name=basic_info.get('fund_name') or fund_code,
                ...
                is_hb=basic_info.get('is_hb'), return_1y=return_1y_val,
                basic_json=_json_dumps(basic_info), performance_json=_json_dumps(performance),
            )
            db.add(basic_record)
        ...
        if trend_record:
            ...
            db.add(FundTrend(
                fund_code=fund_code, net_worth_trend_json=_json_dumps(fresh_nwt),
        ...
        estimate_result = _upsert_fund_estimate(
            db, fund_code, name=estimate.get('name'),
            net_worth=estimate.get('net_worth'), net_worth_date=estimate.get('net_worth_date'),
            estimate_value=estimate.get('estimate_value'), estimate_change=estimate.get('estimate_change'),
        ...
        if isinstance(estimate_result, dict):
            fund_data['realtime_estimate'] = {**estimate,
        ...
        if extra_record:
            ...
            db.add(FundExtraData(
                fund_code=fund_code, holder_structure_json=_json_dumps(extra['holder_structure']),
        ...
        if auto_fallback:
            fund_data["_data_source"] = {
                "mode": "auto", "used": "legacy", "fallback": True,
...
def get_stock_kline(code):
    ...
    try:
        ...
        mds = get_mds()
        result = mds.get_a_stock_kline(normalized_code, klt=period, fqt=adjust, start_date=start_date, end_date=end_date)
...
def get_fund_compare_data(fund_code):
    ...
    try:
        ...
        if use_cache:
            ...
            if data:
                risk_data_valid = (risk_record and is_data_fresh(risk_record.updated_time, days=7)
...
def get_fund_data_versions(fund_code):
    ...
    return jsonify({
        'fund_code': fund_code,
        'basic_info': {'exists': basic is not None, 'updated_time': basic.updated_time.isoformat() if basic and basic.updated_time else None},
        'trend': {'exists': trend is not None, 'updated_time': trend.updated_time.isoformat() if trend and trend.updated_time else None},
        'risk_metrics': {'exists': risk is not None, 'updated_time': risk.updated_time.isoformat() if risk and risk.updated_time else None, 'has_valid_data': risk.sharpe_ratio_1y is not None if risk else False},
        'screening_rank': {'exists': rank is not None, 'updated_time': rank.updated_time.isoformat() if rank and rank.updated_time else None, 'pass_4433': (rank.pass_4433 == 1) if rank else None},
    })
