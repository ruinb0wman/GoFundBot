...
from models import (
    FundBasicInfo, FundRiskMetrics, FundScreeningRank, FundIndustryTag,
    FundPortfolio, FundExtraData, StockIndustry, DataFetchTask, FundNavHistory,
)
from services.helpers import (
    _normalize_fund_code, _json_loads, _json_dumps,
)
from services.stock_utils import _normalize_stock_code
from services.stock_industry import (
    _collect_stock_codes_from_portfolios, warm_stock_industry_dictionary,
    build_stock_industry_dictionary_from_akshare,
)
from services.fund_industry import (
    _screening_industry_context, _upsert_fund_industry_tag,
    batch_refresh_fund_industry_tags, rebuild_industry_performance_stats,
    _industry_performance_payload, _enhance_portfolio_industries,
)
from services.industry_classification import (
    _build_portfolio_industry_tag, _fund_text_industry_fallback,
    _build_industry_tag_from_cached_holdings,
)
from services.screening_engine import (
    screening_update_status, screening_stop_flag, batch_update_fund_data,
    batch_fill_risk_metrics, _latest_active_task, _is_active_task,
    _task_to_update_status, _set_screening_progress, _create_data_fetch_task,
    _save_screening_snapshot_item, _fund_type_lookup, _fetch_screening_snapshot_items,
    calculate_same_type_rankings, update_single_fund_risk_metrics,
    check_4433_rule,
)
...
def get_screening_status():
    ...
    type_counts = {}
    types = db.query(FundBasicInfo.fund_type, func.count(FundBasicInfo.fund_code)).group_by(FundBasicInfo.fund_type).all()
    for t, c in types:
        if t:
            type_counts[t] = c
    ...
    return jsonify({
        'basic_count': basic_count, 'risk_metrics_count': risk_count,
        'ranking_count': rank_count, 'industry_tag_count': industry_tag_count,
        'nav_history_count': nav_history_count, 'pass_4433_count': pass_4433_count,
        'latest_update': latest_update, 'type_counts': type_counts,
...
def start_screening_update():
    ...
    task = _create_data_fetch_task(db, 'screening_update', {
        'fund_types': fund_types, 'limit': limit, 'mode': mode,
        'industry_limit': industry_limit, 'build_industry_dictionary': build_industry_dictionary,
        'tasks': tasks,
    }, message='更新任务已启动')

    thread = threading.Thread(target=batch_update_fund_data,
                              args=(fund_types, limit, mode, task.id, industry_limit, tasks, build_industry_dictionary))
    thread.daemon = True
    thread.start()

    return jsonify({
        'message': '更新任务已启动', 'fund_types': fund_types, 'limit': limit,
        'mode': mode, 'industry_limit': industry_limit,
        'build_industry_dictionary': build_industry_dictionary,
        'tasks': tasks, 'task_id': task.id,
    })
...
def recalculate_rankings():
    ...
    try:
        ...
        ).join(FundScreeningRank, FundBasicInfo.fund_code == FundScreeningRank.fund_code).group_by(FundBasicInfo.fund_type).all()

        type_stats = {}
        for fund_type, total, pass_count in stats:
            if fund_type:
                type_stats[fund_type] = {
                    'total': total, 'pass_4433': pass_count or 0,
...
def get_available_fund_types():
    ...
    if strategy == '4433':
        query = query.join(FundScreeningRank, FundBasicInfo.fund_code == FundScreeningRank.fund_code).filter(FundScreeningRank.pass_4433 == 1)
...
def get_screening_industry_tags():
    ...
    stats = {}
    rows = db.query(FundIndustryTag.industry_tag, func.count(FundIndustryTag.fund_code)).group_by(FundIndustryTag.industry_tag).all()
...
def rebuild_screening_industry_tags():
    ...
    try:
        result = batch_refresh_fund_industry_tags(db, fund_codes=fund_codes, force=force, limit=limit,
                                                   build_full_dictionary=True, allow_missing_stock_network=True)
        rebuild_industry_performance_stats(db)
...
def get_stock_industry_status():
    ...
    with_industry = db.query(StockIndustry).filter(StockIndustry.industry.isnot(None), StockIndustry.industry != '').count()
    ...
    return jsonify({
        'total': total, 'with_industry': with_industry,
...
def warmup_stock_industry():
    ...
    try:
        result = warm_stock_industry_dictionary(db, fund_codes=fund_codes if isinstance(fund_codes, list) else None, force=force, limit=limit)
        db.commit()
    except Exception as exc:
        db.rollback()
        return jsonify({'success': False, 'error': str(exc)}), 500
...
def build_stock_industry_from_akshare():
    ...
    data = request.get_json() or {}
    try:
        result = build_stock_industry_dictionary_from_akshare(db, force=bool(data.get('force')),
...
def query_screening_funds():
    ...
    query = db.query(FundBasicInfo, FundRiskMetrics, FundScreeningRank, FundIndustryTag).outerjoin(
        FundRiskMetrics, FundBasicInfo.fund_code == FundRiskMetrics.fund_code
    ).outerjoin(FundScreeningRank, FundBasicInfo.fund_code == FundScreeningRank.fund_code).outerjoin(
    ...
    for key in ['sharpe_min', 'volatility_max', 'max_drawdown_max', 'calmar_min']:
        ...
        if value is not None:
            cols = {'sharpe_min': FundRiskMetrics.sharpe_ratio_1y, 'volatility_max': FundRiskMetrics.volatility_1y,
                    'max_drawdown_max': FundRiskMetrics.max_drawdown_1y, 'calmar_min': FundRiskMetrics.calmar_ratio_1y}
            op = {'sharpe_min': '>=', 'volatility_max': '<=', 'max_drawdown_max': '<=', 'calmar_min': '>='}
            query = query.filter(getattr(cols[key], f'__{op[key]}__', None) or (cols[key] >= value if key.endswith('_min') else cols[key] <= value))
    ...
    for basic, risk, rank, industry_tag in results:
        ...
        fund_list.append({
            ...
            'industry_tag': ({
                'name': industry_tag.industry_tag, 'ratio': industry_tag.industry_ratio,
                'count': industry_tag.industry_count, 'basis': industry_tag.basis,
...
def get_screening_strategies():
    strategies = [
        {'id': '4433', 'name': '4433法则', 'description': '同类型基金中：近1/2/3年排名前25%，近3/6个月排名前33%', 'tags': ['经典策略', '同类排名', '业绩稳定']},
        {'id': 'high_sharpe', 'name': '高夏普比率', 'description': '夏普比率 > 2，单位风险收益最优', 'tags': ['风险调整', '收益优化']},
        {'id': 'low_volatility', 'name': '低波动策略', 'description': '波动率 < 15%，最大回撤 < 15%，稳健型', 'tags': ['低风险', '稳健']},
        {'id': 'anti_fragile', 'name': '反脆弱策略', 'description': '在极端行情中表现稳健的基金', 'tags': ['抗跌', '极端行情']},
        {'id': 'high_calmar', 'name': '高卡玛比率', 'description': '年化收益/最大回撤比值高，性价比最优', 'tags': ['风险调整', '性价比']},
    ]
...
def get_screening_fund_detail(fund_code):
    ...
    db = get_db()
    result = db.query(FundBasicInfo, FundRiskMetrics, FundScreeningRank, FundExtraData).outerjoin(
        FundRiskMetrics, FundBasicInfo.fund_code == FundRiskMetrics.fund_code
    ).outerjoin(FundScreeningRank, FundBasicInfo.fund_code == FundScreeningRank.fund_code).outerjoin(
    ...
    return jsonify({
        ...
        'returns': {'1m': perf.get('1_month_return'), '3m': perf.get('3_month_return'),
                     '6m': perf.get('6_month_return'), '1y': perf.get('1_year_return'),
                     '2y': perf.get('2_year_return'), '3y': perf.get('3_year_return')},
        'risk_metrics': {
        ...
        'rankings': {'1m': rank.rank_pct_1m if rank else None, '3m': rank.rank_pct_3m if rank else None,
                      '6m': rank.rank_pct_6m if rank else None, '1y': rank.rank_pct_1y if rank else None,
                      '2y': rank.rank_pct_2y if rank else None, '3y': rank.rank_pct_3y if rank else None},
        'pass_4433': (rank.pass_4433 == 1) if rank else False,
