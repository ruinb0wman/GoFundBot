...
export function useFundAIAnalysis<T extends (...args: any[]) => any>(
  ...
  const analyze = async () => {
    ...
    // Build AnalystInput from fund detail (frontend-side; same as Node legacy route)
    const buildInput = async (): Promise<AnalystInput> => {
      ...
      const managers = (detail.sections?.managers?.data?.items ?? []).map((m) => ({
      ...
      return {
        ...
        fundName: basic.name ?? '',
        fundType: basic.type ?? '',
        netWorthTrend: (detail.sections?.navHistory?.data?.items ?? []).map((i) => ({ date: i.date, netWorth: i.nav })),
        riskMetrics: {},
        holdings,
        managers,
        industryTag: classifyFundIndustry(basic.name ?? ''),
      }
    }
    ...
    try {
      ...
      try {
        ...
        const cur = perf?.return1m ?? (perf?.return3m != null ? perf.return3m / 3 : null)
        if (cur != null) {
          await resolvePending(props.fundCode, cur)
        }
      ...
      input.pastContext = pastContext || undefined
      input.strategyContext = strategyContext || undefined

      let gotResult = false
      for await (const ev of analyzeFundStream(input, llmConfig)) {
        if (ev.stage === 'stage') {
          stageMessage.value = ev.content || ''
        } else if (ev.stage === 'result') {
          try {
            const parsed = JSON.parse(ev.content)
            data.value = { ...parsed, analyst_reports: analystReports.value }
          ...
          try {
            const parsed = JSON.parse(ev.content)
            if (parsed && parsed.analyst_role) {
              analystReports.value.push(parsed)
            }
          } catch { /* ignore */ }
        }
      }

      // If stream failed before a supervisor result, run non-streaming once.
      if (!gotResult && !error.value) {
        const full = await analyzeFund(input, llmConfig)
        if (full.supervisor) {
          data.value = full
          analystReports.value = full.reports || []
          storeAnalysis(props.fundCode, full.supervisor)
        }
      }
    } catch (err) {
      error.value = '分析失败: ' + (err?.response?.data?.error || err?.message || String(err))
    } finally {
      loading.value = false
      streamingContent.value = ''
      stageMessage.value = ''
