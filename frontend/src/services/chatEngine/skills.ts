/**
 * Chat skills & intent routing (frontend).
 * Ported from Service `chatSkills.ts`. Prompts embedded as TS strings.
 */
...
5. 纯概念、纯理论问题（如"什么是定投"）不涉及数据查询，直接回答即可
6. 涉及具体市场数据、历史走势、涨跌幅、时间段、指数/基金/个股表现的问题，必须先调用工具获取数据再分析，禁止凭训练数据记忆回答
7. 当前是 2026 年 7 月。涉及"今年""去年""最近""这个月"等相对时间表述时，优先使用工具获取最新数据`
...
4. 回到用户问题：展示数据后，必须从数据回到用户最关心的问题上给出直接回答`
...
4. 区分问题类型：需要具体数据查询的（行情、基金、新闻、回测）才调用工具；概念性、理论性、原理性问题直接回答，不调任何工具`
...
const ROUTER_SYSTEM_PROMPT = `你是一个意图分类器。根据用户的问题，从以下类别中选择最匹配的一个，**只回复类别名称**：
...
export class SkillRouter {
  ...
  keywordRoute(message: string): string | null {
    ...
    for (const name of priority) {
      ...
      if (skill && skill.keywords.some((kw) => message.includes(kw))) {
        return name
      }
    }
    return null
  ...
  async route(message: string, preferred?: string): Promise<Skill> {
    if (preferred && SKILL_MAP[preferred]) return SKILL_MAP[preferred]
  ...
  async llmRoute(message: string): Promise<string> {
    try {
      ...
      const name = response.content.trim().toLowerCase()
