      ...
      <div class="stock-section chart-section">
        ...
        <div class="chart-wrapper" v-else-if="filteredKlineData.length > 0">
          ...
          <div class="chart-summary" v-if="klineSummary">
            <div class="summary-item"><span class="summary-label">{{ '区间涨幅' }}</span><span class="summary-value" :class="klineSummary.changePercent >= 0 ? 'up' : 'down'">{{ klineSummary.changePercent >= 0 ? '+' : '' }}{{ fmtNumber(klineSummary.changePercent, 2) }}%</span></div>
            <div class="summary-item"><span class="summary-label">{{ '起始价' }}</span><span class="summary-value">{{ fmtNumber(klineSummary.startPrice, 2) }}</span></div>
            <div class="summary-item"><span class="summary-label">{{ '最新价' }}</span><span class="summary-value">{{ fmtNumber(klineSummary.endPrice, 2) }}</span></div>
            <div class="summary-item"><span class="summary-label">{{ '最高' }}</span><span class="summary-value high">{{ fmtNumber(klineSummary.high, 2) }}</span></div>
            <div class="summary-item"><span class="summary-label">{{ '最低' }}</span><span class="summary-value low">{{ fmtNumber(klineSummary.low, 2) }}</span></div>
          </div>
        </div>
