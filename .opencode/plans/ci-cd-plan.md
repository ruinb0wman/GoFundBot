...
| 步骤 | 内容 | 检查点 | 状态 |
|------|------|:------:|:----:|
| ✅ 1.1 | 安装 ruff，在 `Backend/pyproject.toml` 配置 lint + format 规则 | `ruff check Backend/` 通过 | ✅ |
| ✅ 1.2 | `ruff check --fix` + `ruff format` 修复全部问题 | `ruff check Backend/` 无报错，65 files formatted | ✅ |
...
| 步骤 | 内容 | 检查点 | 状态 |
|------|------|:------:|:----:|
| ✅ 2.1 | 编写 `DataService/Dockerfile` (node:18-alpine, 多阶段) | `docker build DataService/` 成功 | ✅ |
| ✅ 2.2 | 编写 `Frontend/Dockerfile` (node:18-alpine, 多阶段) | `docker build Frontend/` 成功 | ✅ |
| ✅ 2.3 | 重写 `Backend/Dockerfile` (Python 3.11-slim, gunicorn) | `docker build Backend/` 成功 | ✅ |
...
| 步骤 | 内容 | 检查点 | 状态 |
|------|------|:------:|:----:|
| ✅ 3.1 | 扩展 `Backend/config.py:validate()` — LLM_API_KEY 必填 | 缺少时 `ConfigValidationError` | ✅ |
| ✅ 3.2 | `DataService/src/index.ts` 添加 `validateEnv()` | 生产环境 CORS 空值时 warn | ✅ |
| ✅ 3.3 | 修复 `DataService/.env.example` (CORS_ORIGINS 拼写、补充缺失项) | 无密钥明文 | ✅ |
...
| ✅ 4.3 | CI — build job (DataService tsc + Frontend vite build) | 构建产物生成 | ✅ |
...
| ruff format Backend/ --check | ✅ 65 files already formatted |
| pre-commit run --all-files | ✅ All hooks passed |
| Backend 单元测试 | ✅ 114 tests OK |
| DataService 类型检查 | ✅ tsc --noEmit 通过 |
| DataService 单元测试 | ✅ 67 tests OK (6 files) |
| Frontend 单元测试 | ✅ 24 tests OK (5 files) |
| docker build DataService/ | ✅ 镜像构建成功 |
| docker build Frontend/ | ✅ 镜像构建成功 |
| docker build Backend/ | ✅ 镜像构建成功 |
